window.CR_DASHBOARD_DATA = [
  ...
  },
  {
    ...
    "status": "Review：静态技能格改为窗口相对 ROI；五窗口截图校验与 focused tests 通过，待真实删除闭环",
    "kind": "review",
    "domain": "窗口/Runner",
    "files": "`SummonSkillService`, `status_sealed1.png`, `status_unobtained1.png`, `status_inactive1.png`, summon-skill slot replay/tests",
  ...
  },
  {
    ...
    "kind": "review",
    "domain": "窗口/Runner",
    "files": "`FiveRingTaskV2`, `WindowTaskRunner`, `WindowRuntimeContext`, `TrackerPanelReaderCloudDecisionService`, `TaskTrackerPanelService`, CR190 focused tests, 五环完成/今日上限 dialog templates",
    "summary": "Runner tracker negative gate 已恢复旧业务顺序：positive `TASK_TRACKER_PATHING` 优先，fresh negative 才允许进入 returned-dialog，plain `dialog-visible:STORY` 只唤醒不 terminal。`AMBIGUOUS/ERROR` fail-closed、post-prepare request gate、高优先级 prepared 阻断均已补；新增对象级 tests 覆盖 STORY wake、runtime consume/wrong/stale/prepared-blocked、post-prepare request gate。两个 reviewer 无 P0/P1/P2；fresh 需验证完成/今日上限 story 不再卡 `RUNNER_PREPARED_NOT_READY`。",
    "verification": "需复核"
  },
  {
    "id": "CR191",
  ...
  },
  {
    ...
    "summary": "新增 `XIULUO_BRAIN` request/response/action 协议和 required-execute 服务骨架；无 local fallback；本地测试只验 parse、identity、TTL、fail-closed，不接生产路径，无 fresh runtime。",
    "verification": "需复核"
  },
  {
    "id": "CR194",
    "number": 194,
    "owner": "worker + 双 reviewer",
    "status": "Planned：dev brain session",
    "kind": "open",
    "domain": "通用",
    "files": "`CloudDecisionDevSidecarService`, `cloud/xiuluo`, `XiuluoBrainDevServerTest`",
    "summary": "实现 dev cloud brain session store：`sessionId/stateSeq/phaseToken/actionId` 防旧响应/重放；云端状态推进不能询问本地 `localNextPhase`。只跑 session/状态测试，无 fresh runtime。",
    "verification": "需复核"
  },
  {
    "id": "CR195",
    "number": 195,
    "owner": "worker + 双 reviewer",
    "status": "Planned：客户端云脑 loop 脚手架",
    "kind": "open",
    "domain": "修罗",
    "files": "`XiuluoTaskV2`, `application.properties`, `XiuluoBrainLoopWiringTest`",
    "summary": "在 disabled flag 后新增修罗云脑 loop。flag off 保持旧路径；flag on 必须 fail-closed 且不得 local fallback。本卡只搭 loop 骨架和 fake-cloud 测试，无 fresh runtime。",
    "verification": "需复核"
  },
  {
    "id": "CR196",
    "number": 196,
    "owner": "worker + 双 reviewer",
    "status": "Planned：phase report / action 模型与 source guard",
    "kind": "open",
    "domain": "修罗",
    "files": "`XiuluoPhaseReport`, `XiuluoActionExecutionResult`, `XiuluoSingleBrainSourceGuardTest`",
    "summary": "定义 enabled path 的 report/facts/action-result 模型，禁止 `XiuluoPhaseReport` 携带 `nextPhase`；source guard 禁止 enabled path 使用 `TASK_POLICY`、`TASK_RECOVERY`、`resolveTaskHotStart`、本地 retry/recover。无 fresh runtime。",
    "verification": "需复核"
  },
  {
    "id": "CR197",
    "number": 197,
    "owner": "worker + 双 reviewer",
    "status": "Planned：hot-start / startup 初始 phase 上云",
    "kind": "open",
    "domain": "修罗",
    "files": "`XiuluoTaskV2`, `XiuluoBrainHotStartWiringTest`, 修罗启动日志",
    "summary": "`resolveTaskHotStart` 拆为 facts collection + `XIULUO_BRAIN` initial command；本地可探测 combat/tracker/dialog/return item，但不能自跳 phase 或提前使用回程道具。Fresh Node A 验启动/accept smoke。",
    "verification": "需复核"
  },
  {
    "id": "CR198",
    "number": 198,
    "owner": "worker + 双 reviewer",
    "status": "Planned：接任务 / objective / tracker shortcut 上云",
    "kind": "open",
    "domain": "导航",
    "files": "`XiuluoTaskV2`, dev brain policy, `XiuluoBrainAcceptTrackerWiringTest`, 修罗 tracker 绿链日志",
    "summary": "接任务、接任务 dialog、accept snapshot、objective、maintenance facts、tracker green shortcut 由云端推进；保留 CR191 顺序。Fresh Node B 验到达 tracker green/pathing。",
    "verification": "需复核"
  },
  {
    "id": "CR199",
    "number": 199,
    "owner": "worker + 双 reviewer",
    "status": "Planned：route / target / enter-battle 上云",
    "kind": "open",
    "domain": "导航",
    "files": "`XiuluoTaskV2`, dev brain policy, `XiuluoBrainRouteEnterBattleWiringTest`, route/target/enter-battle 日志",
    "summary": "`WAIT_TRACKER_SHORTCUT_PATHING`、目标导航、目标 NPC 点击、看打/入战由云端决定 fallback/next phase。本地只报告 route/pathing/click/dialog/combat facts。Fresh Node C 验进入战斗。",
    "verification": "需复核"
  },
  {
    "id": "CR200",
    "number": 200,
    "owner": "worker + 双 reviewer",
    "status": "Planned：combat / return / team-return / recovery 上云",
    "kind": "open",
    "domain": "修罗",
    "files": "`XiuluoTaskV2`, dev brain policy, `XiuluoBrainCombatReturnRecoveryWiringTest`, 修罗回程/归队/timeout 日志",
    "summary": "`WAIT_COMBAT`、回程、归队、post-combat idle、pre-combat watchdog、loop guard、retry/recovery 全部改为上报 facts 后由 `XIULUO_BRAIN` 决定；`team return` 保持本地 detector/fact。Fresh Node D 验一整轮。",
    "verification": "需复核"
  },
  {
    "id": "CR201",
