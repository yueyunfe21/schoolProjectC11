window.CR_DASHBOARD_DATA = [
  ...
  },
  {
    ...
    "number": 208,
    "owner": "谢帅",
    "status": "Review：item 11 fresh P1（`routeMode` 协议不一致）已修——cloud 回包改 canonical `routeMode=`，client 过渡兼容旧 `mode=`；双侧构建通过，fresh 待验",
    "kind": "review",
    "domain": "通用",
    "files": "`TextRecognizer`, `GameTextLineOcrService`, `TaskTrackerPanelService`, `DialogService`, `NpcClickService`, `QuestManagerService`, `PlayerStateService`, `TeamRoleDetectionService`, `LocationVisionService`, `NavigationService`, `RouteCloudDecisionService`, cloud OCR endpoint",
    "summary": "已划掉：1（CR212，fresh 待验）、2（CR209）、7/8（CR213，专项 fresh 待验）及补充白模板项；12/13/14 已明确不迁。2026-07-10 fresh：云端已返回黄色目的地 click，但本地严格 gate 只读 `routeMode`，cloud 回包实际为 `mode=yellow-destination-mini-map`，导致未发物理点击。3/4 仍有本地 OCR fallback；5/6、9-11、15/16 尚未迁或依赖云端 word boxes，父卡不能关闭。",
    "verification": "需复核"
  },
  {
    "id": "CR209",
  ...
  },
  {
    ...
    "number": 250,
    "owner": "Codex",
    "status": "Review blocked：首轮四项已修；仍缺远端动作输入安全门与 action 一次执行契约",
    "kind": "review",
    "domain": "窗口/Runner",
    ...
    "summary": "云端只决定“点哪/看哪”、本地独占 intent/park 已明确；但执行器尚未定义 phase-scoped action allowlist、client 边界/步数限制、InputSequences 原子提交和 `(windowId,requestId,actionId)` 去重，云端 response 超时重放可能重复物理点击。",
    "verification": "需复核"
  },
  {
    "id": "CR251",
    "number": 251,
    "owner": "Codex",
    "status": "Review blocked：首轮四项已修；仍缺 candidate outcome/decision token 与 observation freshness 可执行契约",
    "kind": "review",
    "domain": "通用",
    "files": "`CoordinateHelper`, `NavigationService.navigateInCurrentMap`, external `MINIMAP_LOCATION`",
    "summary": "固定 client-relative 帧、candidateId、6 处 transform 调用表已齐；仍须补 `decisionId`/单次 token、候选 outcome enum、input snapshot 最大年龄与 local retry/replay 规则，避免网络重试把同一点击执行两次或把过期观察送云端决策。",
    "verification": "需复核"
  },
  {
    "id": "CR252",
