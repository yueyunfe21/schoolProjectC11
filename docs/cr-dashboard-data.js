window.CR_DASHBOARD_DATA = [
  {
    ...
    "status": "**SOURCE FIXED / 合同 16/16 / 五环19+修罗40+G124-81 未退化 / FRESH REQUIRED**",
    "kind": "open",
    "domain": "五环",
    "files": "Cloud `FiveRingTaskV3.java`；稳定事实塑形仅在证明必要时触及 `CloudObservationHttpHandler.translateStabilityFact`；事故时序/跨图进店/清 intent/普通导航负回归合同",
    "summary": "**五环买鞋：战斗中误开快捷购买 + 自动进牛记布店后遗留 `STOPPED_AWAY`。** 旧 V2 会先认真实地图“牛记布店”，V3 却在地图检查和 terminal 清理前先跑快捷买鞋并可提前返回；8 月 26 日停稳判定上云又将 `牛记布店(10,7)` 与旧目标 `长安(130,130)` 比成 `STOPPED_AWAY`。修复须同时建立战斗硬门，并让本业务跨图进店事实先结算、清掉旧 intent，禁止泛化普通 `STOPPED_AWAY`。",
    "verification": "需复核"
  },
  {
    "id": "G124",
    "number": 124,
    "owner": "Claude",
    "status": "**SOURCE FIXED / Cloud 生产编译 exit 0 / 新合同 86/86 / 变异 7/7 全杀 / 修罗整包 40/40 未退化 / 待外部 Review / FRESH REQUIRED**",
    "kind": "review",
    "domain": "修罗",
    "files": "Cloud 新增框架包 `com.bot.dhxy.runner.progress`（`TaskRunProgressLedger`/`TaskProgressReport`/`TaskProgressGuard`）、`CloudWholeTaskRuntimeLocalServiceClient.updateProgress` 唯一出口、七个任务 `XiuluoTaskV2`/`GhostKingTask`/`CatchGhostTask`/`WubeiTask`/`XinshouTrainingTask`/`FiveRingTaskV3`/`TiantingTask`；三个 G124 合同 + 两个既有合同刷新",
    "summary": "G077 覆盖漏项的**通用**修复：暂停恢复带回的累计次数此前只有天庭消费，其余六任务硬编码 `0`，22:02 修罗把 UI 从 13/90 覆盖成 0/90。轮次收成框架唯一真源，累计次数与“本进程第一轮”拆成两个独立字段（不再由 `completedRuns == 0` 兼任），上限按累计值比 maxRuns（13/90 恢复后只再跑 77 轮）。防复发双门：裸整数上报重载从 API 删除（编译期杜绝），唯一出口再过纯函数护栏，低于基线时抬回基线 + ERROR 留痕（不抛不丢）。天庭改用同一账本，全仓 `getInitialCompletedRuns()` 只剩一个解析面，杜绝双计。",
    "verification": "需复核"
  },
  {
    "id": "G123",
    "number": 123,
    "owner": "Codex",
    "status": "**用户已重写方案 / NO CODE / 仅允许 Git 旧版最小回迁 / 待实施**",
    "kind": "open",
    "domain": "修罗",
    "files": "Cloud `CloudUiCleanerPort.java`、`DialogService.java`；G123 定向合同与事故 marked 图",
    "summary": "逐分支恢复 `9aa987d1` 的 `forceCloseDialog`：NO_DIALOG、受旧角色门保护的 STORY、OPTION `fallbackLastOption`。唯一适配是直接消费 Runner 同帧快照并让 fallback 使用 supplied `DialogDetection`，force-close 内零 CAPTURE；禁止修改 Client、Runner、修罗、NPC/CTRL 或协议。",
    "verification": "需复核"
  },
  {
    "id": "G122",
    "number": 122,
    "owner": "Claude(mavenproject-3d)",
    "status": "**REVIEW #2 PASS：P0=P1=P2=0，源码/合同通过，仅待统一 fresh**",
    "kind": "review",
    "domain": "通用",
    "files": "Client `WindowObservationSampler`、`DialogService`、`NpcArrivalFrameFifoLocalExecutor`；Cloud `SmartClickRecognizer`、`NpcClickSmartQueueStore`、`NpcClickMemoryStore`、`DalisiQuizTask`；两仓 G122 合同与事故 fixture",
    "summary": "大理寺五开 NPC 入口四重修复已闭环：fresh exact-intent 坐标可直接 ARRIVED；CONTINUATION 不再被吞且换帧有界；defer 待定态如实上报、`success=false`，真实 accept 后才结算；拒收帧不污染证据池。外部二审 P0=P1=P2=0，剩余仅五角色 fresh 与原图 marked 验收。",
    "verification": "需复核"
  },
  {
    "id": "G121",
    "number": 121,
    "owner": "Claude",
    "status": "**合同错已修，生产零改动，12/12 全绿，待复审**",
    "kind": "open",
    "domain": "通用",
    "files": "Cloud `NpcArrivalFrameQueueStoreContractTest.producerFirstPublishesCtrlReadyOnlyAfterUnlockAndExactlyOnce`",
    "summary": "带 `ctrlProbePoints` 的候选是生产者屏障（javadoc 明写），push 会就地 park；原用例把 `countDown` 排在该 push 之后 → 闩永不减 → await 恒 false，而 unlock 在 await 之后，测试把自己写成死锁。照本文件既有惯例改为入口 countDown + 300ms 有界静置断言。变异（去掉 unlocked 门）1 红，断言仍有牙齿。",
    "verification": "需复核"
  },
  {
    "id": "G120",
    "number": 120,
    "owner": "Claude（评审方派驻子代理，继 G119 同一实施方）",
    "status": "**IMPLEMENTED / 隔离编译 exit 0 / 合同 6/6（整包 40/40×2）/ 变异 3/3 全杀 / 形状取舍待用户拍板 / 待外部 Review / FRESH BLOCKED**",
    "kind": "review",
    "domain": "通用",
    "files": "Cloud `XiuluoTaskV2.resumeRouteAfterIncidentalCombat`（单一决策源重构，+62 行；共享服务零改动）、新合同 `G120IncidentalExitIncenseContractTest`（6 用例）",
    "summary": "三出口各恰好一个香载体成为决策 record 的构造不变量（零载体=事故、双载体=二次恢复，构造期即拒绝）：两个续航出口当场 `ensureSheYaoXiangActiveForLeaderTask`（present 只查不补、absent/≤20m 立即补走既有语义），title 消失出口唯一香检骑既有 `CLIENT_RUNNER_EXIT` 延迟恢复的安全点消费——不重开已撤回的 AutoCombatService 标志。待拍板：鬼王同款\"查香先于 title 仲裁\"排序与严格恰好一次互斥，本卡按严格恰好一次实施。",
    "verification": "需复核"
  },
  {
    "id": "G119",
    "number": 119,
    "owner": "Claude（评审方派驻子代理，续作 mavenproject-29 调查）",
    "status": "**IMPLEMENTED / 隔离编译 exit 0 / 合同 11/11（整包 34/34×2）/ 变异 4/4 全杀 / 上限数值待用户拍板 / 待外部 Review / FRESH BLOCKED**",
    "kind": "review",
    "domain": "窗口/Runner",
    "files": "Cloud `XiuluoTaskV2`（entry 锁存 + exit 结算 + accept 双复位，+105 行）、新增 `XiuluoIncidentalCombatBudgetClock`（补偿账本，185 行）、新合同 `G119IncidentalCombatBudgetCompensationContractTest`（11 用例，含 01:47-01:50 事故逐毫秒回放）",
    "summary": "偶遇战退战按 exact Runner 边沿事件 `createdAtMs` 差补回 pre-combat 预算（消费迟到/UI 清理仍计入）；补偿按 `300_000ms`/attempt 封顶，花完后野怪时间重新计入、轮次走既有 same-round restart——连续野怪不能无限续命。latest-only 事实面下交错代次只会少补（fail 向更早超时），不重建已撤回的 journal/ACK 设施。",
    "verification": "需复核"
  },
  {
    "id": "G118",
    "number": 118,
    "owner": "Claude",
    "status": "**USER APPROVED / 实现完整 / 合同 3/3 / 生产零改动 / 待复审**",
    "kind": "open",
    "domain": "通用",
    "files": "Cloud `CloudWholeTaskObserver` 五环 title-rearm 生命周期（修 A 源头作废 + 修 B 存量收回）与 prepared-action 唯一槽",
    "summary": "修 A/修 B/package-private 判定 helper/六向真值表均已在树；本轮补齐卡上要求的跨轮时序合同：round1 收尾 → rearm 迟到 → round2 链已激活，迟到的 rearm 不得重新占槽（修 B 拦不住后来的，判定必须在发布前拒绝）。变异双向全杀（恒不退休 2 红 / 恒退休 1 红）。",
    "verification": "需复核"
  },
  {
    "id": "G117",
    "number": 117,
    "owner": "Codex",
    "status": "**已实施并上线，待用户追认**（卡片曾误记为『未改生产/待批准方案』）：生产已改、两个合同已加、fixture 已固化、`UICleanerService.class` 01:41:58 编译、客户端 01:54 重启后已按 x-only 运行",
    "kind": "open",
    "domain": "Dialog",
    "files": "Client `UICleanerService` 通用关窗模板池收窄为 `x\\d*\\.png`；命中日志补 `template=`；孤儿 `images/template/cancel/quxiao.png` 移入 `quarantine/`",
    "summary": "事故非孤例：误点『取消任务』区共 **21 次分 7 组**（00:30–00:38），卡原文只记了 00:34 一组。引入点＝`quxiao.png` 于 08-28 18:01 被丢进通用目录（未跟踪）。修复后该文件生产读者数为 0，业务流程读的是 `dialog/guiwang/` 与 `dialog/tianting/` 下的同名文件。待鬼王一轮 fresh 验收。",
    "verification": "需复核"
  },
  {
    "id": "G116",
    "number": 116,
    "owner": "Claude（补编号）",
    "status": "评审 RETURN 后**返修完成**：P0-2（unlock 失败推进锁定帧、打死 replacement）已修并加回归合同；P0-1/P1-5 按用户裁决**撤回**客户端 30 秒墙钟上界（恢复 `exact WAIT 不设墙钟到期` 原有约束）；三个被打红的既有合同已恢复为绿",
    "kind": "open",
    "domain": "通用",
    "files": "Cloud `NpcClickSmartQueueStore` 到达帧开锁、`CloudObservationHttpHandler` 门结果",
    "summary": "**30 秒客户端上界属未批准行为，现已撤回**：`NpcArrivalFrameFifoLocalExecutor.java` 当时整文件回退至 HEAD（`git diff HEAD` = 0）；**注意该文件之后又被 G122 P1-3 再次修改（+124 行），所以「= HEAD」这句从 2026-08-29 起不再成立**——墙钟上界确认未回流、功能无恙，但下一位评审不要据此判定文件未改（外部评审 P2-2）。`git diff HEAD` = 0。生产链超时继续由云端既有 `PRODUCER_CHAIN_WAIT_TIMEOUT_MS = 120_000` 负责。合同 6/6；同族 11/12（余 1 为 HEAD 基线红，已另立 G121）。",
    "verification": "需复核"
  },
  {
    "id": "G115",
    "number": 115,
    "owner": "G115 实施者",
    "status": "终审通过 P0=P1=P2=0，SOURCE+CONTRACT APPROVED；部署被批次污染阻塞：clean 重建会编入 26 个生产文件（G065/G078/G106/G107/G108/G109/G110/G111/G112/G113/G114/G116），待 manifest 逐项放行后才可停机重建 fresh",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "五环 Tracker 面板矩形与裁后原点、裁剪日志、观察面备动作静默门与 negative read 原因",
    "summary": "任务栏贴窗口左边致 panelLeft=-1 被一道边界门整块拒绝，绿链列表为空使 3519 空转十分钟；该门在 title 真门之前返回且从不防锚点错位，整条删除改为与窗口求交后共用裁后原点，判据收敛为 anchor→title→links。",
    "verification": "需复核"
  },
  {
    "id": "G114",
    "number": 114,
    "owner": "G114 实施者",
    "status": "范围回退完成（用户定案 B）：仅保留最初批准的方案 B（失败恢复使用 `outcome.nextState()` 所属 exact attempt）；四文件回 HEAD、7 个扩展合同删除、2 份方案 B 合同保留；待一次针对性复审 / FRESH BLOCKED",
    "kind": "open",
    "domain": "通用",
    "files": "Cloud `XiuluoTaskV2` 失败恢复代际解析 `resolveExactAttemptResetContext`；回退后 `CloudWholeTaskReadyEventState` / `WindowReadyEvent` / `CloudFastExpectedCombatExitCoordinator` / `AutoCombatService` 与 HEAD 一致",
    "summary": "偶遇战退战后新 attempt 已接管、旧 watchdog 同拍到期时，恢复必须打向产出 FAILED 的那一代并回到同一第 5 轮重接，不得 terminal FAILED 提前切鬼王；后来扩出的 combat-edge journal / ACK / overflow / 补偿账本已整体撤回，拆为 G119、G120。",
    "verification": "需复核"
  },
  {
    "id": "G113",
    "number": 113,
    "owner": "Codex Review",
    "status": "REVIEW #5 通过（原答题链）；2026-08-29 追加的 NPC 入口假成功问题已由 **G122** 接管，不再作为 G113 返修项；原范围待 fresh",
    "kind": "review",
    "domain": "Dialog",
    "files": "大理寺题目识别、选项分割、题库学习、`accept.png` 答题入口硬门",
    "summary": "G113 原有答题链保留；`DEFERRED_TO_TASK` 假阳性、到达帧续帧丢点与 `openOfficialDialog` 丢弃点击结果属于 G122 的统一入口合同，禁止两卡重复修改。",
    "verification": "需复核"
  },
  {
    "id": "G112",
    "number": 112,
    "owner": "Codex Review",
    "status": "REVIEW #4 通过：P0=P1=P2=0，Client 9/9、Cloud 5/5；待双端重启 fresh",
    "kind": "review",
    "domain": "通用",
    "files": "鬼王取消恢复 phase、NPC FIFO defer 合同、取消 Dialog、replay/回归合同",
    "summary": "三类真实图、取消模板框/红点、四态 fail-closed、共享 evidence 合同均闭环；重启两端后验收 NPC 零误点与无自循环。",
    "verification": "需复核"
  },
  {
    "id": "G111",
    "number": 111,
    "owner": "实施 agent（Codex 复审通过）",
    "status": "源码通过，合同 5/5、回放 28/0；待重启 Cloud fresh",
    "kind": "open",
    "domain": "通用",
    "files": "五环 Tracker 下划线识别、无下划线 fail-closed、自包含 replay 合同",
    "summary": "fallback 已删除，测试与 fixture 可跟踪且自包含；共享 `target/classes` 在运行期再次被覆盖，重启前禁止 fresh。",
    "verification": "需复核"
  },
  {
    "id": "G110",
    "number": 110,
    "owner": "Codex",
    "status": "源码已修、生产编译通过、隔离合同 3/3，待 3473 fresh",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "五环 V3 完成策略、轮次状态、进度上报、回归合同",
    "summary": "第 1 轮 `finishedOnce` 已改为非终态交接并进入 `round=2 / PREPARE`；仅全部完成才结束窗口任务。",
    "verification": "需复核"
  },
  {
    "id": "G108",
    "number": 108,
    "owner": "Claude",
    "status": "**台账行此前过期已订正**：五项返修 2026-08-27 即已落地（正文为准）。2026-08-29 独立复验：五条逐条在代码中确认、合同 25/25、变异 8/8 全杀、生产代码零改动；当前状态 `RE-DELIVERED 待外部 REVIEW #2`",
    "kind": "review",
    "domain": "通用",
    "files": "Cloud `LeaderTeamReturnCoordinator` gate 开窗与 AWAIT_HUD_EVIDENCE、`TeamReturnZhaoWatchState` epoch 围栏与严格解析、`CloudWholeTaskObserver` terminal releaseRun + removeInterest",
    "summary": "本轮补 1 条合同堵覆盖缺口：原有三条 P1-1 合同都在 setup 里预先开窗，**没有任何合同钉住『生产入口自己会开窗』**——而 Codex 指的缺陷恰恰发生在没人开过窗的 hot-start 入口。今日 5 小时有效日志实证：开窗 190 / AWAIT 停等 169 / fresh ABSENT 收门 168 / HUD 否决 0 / 180s fatal 0 / 面板开合循环 0；hot-start 路径于 `00:12:02` 真实命中该门。停等成本 p50 3.01s。",
    "verification": "需复核"
  },
  {
    "id": "G107",
    "number": 107,
    "owner": "Claude",
    "status": "**二审两条 P1 已修并经外部复审关闭**（独立复跑 4/4 + 9/9）；新开一条非阻断 P2：`CaptureResult` 缺可审计新鲜身份，须在 fresh 验收前补齐或经用户豁免",
    "kind": "open",
    "domain": "通用",
    "files": "Cloud `AutoBattleTask` 两道门拆分、`LeaderDeathRecoveryService` 有界证据等待、`TaskMaintenanceService` 上报唤醒、`GhostKingTask` 单一调用点；Client `TeamReturnPanelLocalOperation`",
    "summary": "团灭判定曾因成员报告晚到 68 秒而降级 DEAD_FALLBACK。拆门后只读上报不再等队长广播，点击仍留权限门后；20 秒有界事件等待放在鬼王调用点，`detectLeaderDeath` 的 never blocks 契约与另外三个调用方零改动。新 P2 的要害：管线级冻结会让面板与 HUD 两路 ROI 同时读到陈旧像素。",
    "verification": "需复核"
  },
  {
    "id": "G106",
    "number": 106,
    "owner": "Codex",
    "status": "REPAIR #2 已交付（P1×2/P2×3 全修），**fresh 已跑**（Client `target/classes` 01:41:58、Cloud 01:53:15-25 全量重建并重启），待 REVIEW #3 判读",
    "kind": "review",
    "domain": "通用",
    "files": "启动脚本四条解析面、preflight、OCR 身份",
    "summary": "三条 Review #2 修复已现场可证（`local-stack-cloud.out.20260829-015327.log`：project root 同级推导 / java runtime 由 preflight 传入 / `model=rapidocr-3.8.1`）。剩余为口径问题：`mvn` 是否算第五面、换机验收门是否真跑一次、`lib-process-tree.ps1` 归卡、`application.properties` 的 `\\uXXXX` 重编码是否声明。",
    "verification": "需复核"
  },
  {
    "id": "G105",
  ...
  },
  {
    ...
    "number": 99,
    "owner": "Codex",
    "status": "核心三通道解耦已交付；fresh 暴露的截图风暴已由 **G103** 接管修复，G099 不再单独返修；随 G103 fresh 一并验收",
    "kind": "open",
    "domain": "通用",
    "files": "Runner 观察采样、输入执行、HTTPS transport 三通道解耦",
    "summary": "G099 保留架构交付；“transport 回包绕过 cadence、驱动整窗截图”是后续 G103 的限频范围，禁止在 G099 重复实现。",
    "verification": "需复核"
  },
  {
    "id": "G098",
  ...
  },
  {
    ...
    "number": 80,
    "owner": "Codex",
    "status": "P1 源码已修、离线验收通过，待 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "Runner 共享帧、pathing movement、位置缓存、replay 证据",
    "summary": "删除固定 `45x12` 横向框；生产方法动态裁括号内完整坐标，用户 marked 图确认及五张多地图 replay 通过。",
    "verification": "需复核"
  },
  {
    "id": "G079",
    "number": 79,
    "owner": "Codex",
    "status": "P1 源码已修并编译通过，待 fresh 验证 `100ms` 补给右键",
    "kind": "open",
    "domain": "通用",
    "files": "公共 maintenance 窗口、Runner 缓存急救、共享补给右键",
    "summary": "公共消费者已可达；人物/宝宝 HP/MP 右键由 `0ms` 改为 `100ms` hold，本轮不新增 post-action 复拍。",
    "verification": "需复核"
  },
  {
    "id": "G078",
    "number": 78,
    "owner": "Codex",
    "status": "**半帧 P1 已闭**（08-28 19:22–23:33 现场 9/9 正向，且 106ms 半帧形态被真实拦下 1 次）；剩余＝完整归队链 fresh + 死亡/团灭场景 fresh 各一次",
    "kind": "open",
    "domain": "通用",
    "files": "通用归队协调器，六个队长任务共用",
    "summary": "协议已泛化（`GHOST_KING_TEAM_RETURN_PANEL_*` 全仓 0 处）；归队门 180s 上界 `GATE_WAIT_TIMEOUT_MS` 超时抛 `TaskFatalException`。死亡链 0 样本，必须跑一次团灭才能关卡。  **另接 G107 划转的 P2：capture 缺可审计新鲜身份（管线冻结会让面板与 HUD 双路同时读陈旧像素），fresh 前须补单调捕获代或经用户豁免。**",
    "verification": "需复核"
  },
  {
    "id": "G065",
    "number": 65,
    "owner": "Claude",
    "status": "**DEPRECATED（用户 2026-08-29 确认）**：成员 pending 第三道 veto 永久退出当前归队合同；不得继续修 E70/E71 或恢复 veto",
    "kind": "deprecated",
    "domain": "通用",
    "files": "历史诊断：成员 return-pending 登记/注销；现行完成门归 G078 + G108",
    "summary": "当前只允许 pending 作为日志诊断，不参与放行。有效归队完成判据为队长面板 `ALL_RETURNED` + G108 HUD fresh `ABSENT`；本卡不再阻塞 Review、构建或 fresh。",
    "verification": "需复核"
  },
  {
    "id": "G077",
    "number": 77,
    "owner": "Codex",
    "status": "双端生产编译与 5 项独立合同通过，待双端重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "暂停进度快照、Client/Cloud 启动协议、天庭累计次数",
    "summary": "暂停仍清瞬时状态，但 UI 保留累计次数；同任务同总数热恢复从原次数继续并按剩余次数收尾。",
    "verification": "需复核"
  },
  {
    "id": "G076",
    "number": 76,
    "owner": "Codex",
    "status": "P1 直接故障已修，待 fresh 验证光牛的滑子实际补蓝",
    "kind": "open",
    "domain": "通用",
    "files": "本地队员 Runner 脱战门、缓存补给计划、共享右键 hold",
    "summary": "自身脱战 `1500ms`、缺蓝识别及公共消费者已通过；共享右键改为 `100ms` hold，不再按下即释放。",
    "verification": "需复核"
  },
  {
    "id": "G075",
    "number": 75,
    "owner": "Codex",
    "status": "源码、实机图回放、合同与 Cloud 编译通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "蟠桃园暗雷巡逻点、点击序列合同",
    "summary": "保留第一下老右点，第二下点击右侧镜像新点，之后在两点间交替。",
    "verification": "需复核"
  },
  {
    "id": "G074",
    "number": 74,
    "owner": "Codex",
    "status": "源码、合同与隔离 Cloud 编译通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "战后补给截图时机、实际右键、`elapsedMs`",
    "summary": "即时战后恢复先稳定等待 `1500ms` 并记录实际耗时，再执行任何恢复截图。",
    "verification": "需复核"
  },
  {
    "id": "G073",
    "number": 73,
    "owner": "Codex",
    "status": "两处源码、契约与 Cloud 编译通过，待 fresh",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "五环战后恢复、任务栏 ROI 战斗保护、Runner/Radar 权威",
    "summary": "战后恢复及 ROI 变化均不再调用 Radar，统一等待 exact Runner 战斗事实。",
    "verification": "需复核"
  },
  {
    "id": "G072",
    "number": 72,
    "owner": "Codex",
    "status": "P1 源码、契约与 Cloud 编译通过，待双端重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "独立自动战斗 Runner 脱战、战后人物/宝宝补给",
    "summary": "战中只维护面板；exact 脱战后立即跑既有 `FULL_RECOVERY`，输入失败保留补给额度。",
    "verification": "需复核"
  },
  {
    "id": "G071",
    ...
    "kind": "open",
    "domain": "通用",
    "files": "天庭 60/60 收尾、回程道具落地验证、步行 fallback",
    "summary": "22:55 是最终收尾回城，不是重新接第 61 次任务。",
    "verification": "需复核"
  },
  {
    "id": "G070",
    "number": 70,
    "owner": "Codex",
    "status": "源码、合同、编译与活动 runtime fresh 通过",
    "kind": "open",
    "domain": "通用",
    "files": "天庭开打结果、场外 watchdog、Runner 战斗边顺序",
    "summary": "开打已执行后等待 exact 入战，禁止 watchdog 抢跑回城。",
    "verification": "需复核"
  },
  {
    "id": "G069",
    "number": 69,
    "owner": "Codex",
    "status": "源码与针对性合同通过，待安全重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "全回程道具任务首轮 Hot Start；补天庭与五倍缺口",
    "summary": "Tracker 未接管且即将接任务时，先处理专属回程道具；stop 仍全清。",
    "verification": "需复核"
  },
  {
    "id": "G068",
    "number": 68,
    "owner": "Codex",
    "status": "P1 源码与针对性合同通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "队列 child exact 观察身份、终态恢复 queue suffix",
    "summary": "child 切换会重发 exact startup 身份；天庭失败仅恢复 `[TIANTING]` 后缀，已成功鬼王不再重做。",
    "verification": "需复核"
  },
  {
    "id": "G067",
    "number": 67,
    "owner": "Codex",
    "status": "**DEPRECATED：已由 G078 + G108 取代**；106ms 半帧形态已在后卡现场被真实拦截，不再单独返修或测试",
    "kind": "deprecated",
    "domain": "通用",
    "files": "历史：鬼王归队末尾 `Alt+T` 半帧误判",
    "summary": "有效实现与验收归 G078 的面板稳定门和 G108 的 HUD fresh-ABSENT 门；禁止沿 G067 恢复旧单门判定。",
    "verification": "需复核"
  },
  {
    "id": "G066",
    "number": 66,
    "owner": "Codex",
    "status": "双仓编译通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "天庭/鬼王标题十秒上限、鬼王专用取消协议与 `quxiao.png` 执行链",
    "summary": "鬼王接取不确定时先真实取消再重接；禁止抓鬼模板和通用最后项冒充取消。",
    "verification": "需复核"
  },
  {
    "id": "G064",
    "number": 64,
    "owner": "Codex",
    "status": "双仓编译与隔离合同通过，待 fresh",
    "kind": "open",
    "domain": "导航",
    "files": "UI 领取双倍开关、exact-run 设置协议、Cloud 双重领双门",
    "summary": "关闭后不识别双倍、不生成待办、不导航长安或点击一品侍卫；手动 G056 保留。",
    "verification": "需复核"
  },
  {
    "id": "G063",
    "number": 63,
    "owner": "Codex",
    "status": "五窗 fresh 通过；继续 100 轮后台监控",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "头像锚点同队分组、启动协议、Cloud scoped 广播与窗口独立自动战斗刷新",
    "summary": "五窗同组广播稳定；自动战斗 30 秒防抖改为 exact `windowId`，3511 与其余四窗同场均完成 `Alt+8`。",
    "verification": "需复核"
  },
  {
    "id": "G062",
    "number": 62,
    "owner": "Codex",
    "status": "首次整窗开包 fresh 通过；回城验收被 G069 阻断",
    "kind": "open",
    "domain": "通用",
    "files": "首次整窗锚点、per-window 缓存 ROI、缓存 miss 整窗重定位",
    "summary": "18:51 首次整窗找到 `(574,411)` 并成功使用摄妖香；天庭启动漏掉接任务前回程道具探测。",
    "verification": "需复核"
  },
  {
    "id": "G061",
    "number": 61,
    "owner": "Codex",
    "status": "源码与 Cloud 编译通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "鬼王回城后 NPC SmartClick、expected-dialog 前检、FIFO 启动",
    "summary": "defer 请求已跳过重复整图 option 前检，固定点 FIFO 与任务专属 probe 保持原序。",
    "verification": "需复核"
  },
  {
    "id": "G060",
    "number": 60,
    "owner": "Codex",
    "status": "两层异常恢复与鬼王内部收口编译通过，待双端 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "Turn 单槽重发、Cloud 终态诊断、热恢复 run、鬼王事件等待与同轮重置",
    "summary": "鬼王删除接任务固定 `5s` 失败门；前九次失败回接任务起点，第十次连续失败终止。Cloud `FAILED/SKIPPED` 仍按原配置热恢复。",
    "verification": "需复核"
  },
  {
    "id": "G059",
    "number": 59,
    "owner": "Codex",
    "status": "源码、双仓编译与隔离连通测试通过，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "UI 设置启动快照、双仓启动协议、Cloud per-run 配置读取",
    "summary": "维护、前置、盒子和补给设置随 exact task run 发送，禁止 Cloud 默认值覆盖 UI。",
    "verification": "需复核"
  },
  {
    "id": "G058",
    "number": 58,
    "owner": "Codex",
    "status": "源码已实现，待鬼王 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "鬼王摄妖香同帧双倍待办、100 分钟 CD、移除鬼王盒子入口",
    "summary": "双倍低于 20 分钟只登记，下一次接任务成功后领取；鬼王不检测或消费 common box。",
    "verification": "需复核"
  },
  {
    "id": "G057",
    "number": 57,
    "owner": "Codex",
    "status": "源码返修完成，待 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "鬼王移动途中识别长寿村、下一共享帧饱和度判定、exact-window `Alt+C`",
    "summary": "已移除稳定坐标依赖；两张现场图地图标签回放 `0.9928/0.9943`，G016 顺序合同 `1/1 PASS`。",
    "verification": "需复核"
  },
  {
    "id": "G053",
    "number": 53,
    "owner": "Codex",
    "status": "源码返修与隔离合同通过，待 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "天庭所有失败重接入口、取消专用本地探针、取消后重接",
    "summary": "跨 round 保留取消栅栏；失败重接且旧 title 在时必须先取消，禁止旧 Tracker 再次执行。",
    "verification": "需复核"
  },
  {
    "id": "G052",
  ...
  },
  {
    ...
    "number": 51,
    "owner": "Codex+Banach",
    "status": "完整目录已跑，52/72 通过，20 项待修",
    "kind": "open",
    "domain": "通用",
    "files": "世界地图拼音首字母、候选下拉 OCR、持久化相对坐标记忆、exact ARRIVED 裁决",
    "summary": "72 项真实后台链全部完成；Alt 粘滞已加释放栅栏且未复现锁定，20 个 OCR/候选确认失败项已有逐项目录证据。",
    "verification": "需复核"
  },
  {
    "id": "G050",
    "number": 50,
    "owner": "Codex",
    "status": "意外脱战 title 仲裁已修，待重启 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "鬼王独立任务、阎王书房导航、地藏王点击、本地接任务 probe、脱战恢复仲裁",
    "summary": "`INCIDENTAL COMBAT_EXITED` 后本地查鬼王 title：存在才复用缓存绿链；不存在则清缓存并按真实任务战斗完成恢复。",
    "verification": "需复核"
  },
  {
    "id": "G049",
  ...
  },
  {
    ...
    "number": 41,
    "owner": "Codex",
    "status": "源码已修，待 fresh：中文输入焦点与 absolute mouse dead-letter",
    "kind": "open",
    "domain": "通用",
    "files": "FakerInput 设备桥、absolute HID 闭环、角色感知输入路由、中文路线输入",
    "summary": "路线输入使用单个 frozen mixed-input 队列事务；鼠标按真实落点闭环纠偏。双仓编译通过，须重启双端现场验收。",
    "verification": "需复核"
  },
  {
    "id": "G040",
  ...
  },
  {
    ...
    "number": 29,
    "owner": "Codex",
    "status": "SOURCE REPAIRED / 待 fresh",
    "kind": "open",
    "domain": "通用",
    "files": "`DialogService`、`TiantingTask`、封妖符结果顺序、定向合同",
    "summary": "已改为等待并钉住 post-FENGYAO Tracker 基线后才允许点坐标；拿符前旧图不再参与完成判断。",
    "verification": "需复核"
  },
  {
    "id": "G027",
  ...
  },
  {
    ...
    "number": 212,
    "owner": "父级 + worker",
    "status": "Source+Test Passed：队员识别与自动战斗实例化/UI投影已修，fresh runtime 待验",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "`CloudTeamRolePreflightService`, `AutoBattleTask`, `CloudTurnTaskRuntime`, `TurnTaskStartAck`, `WindowTaskControlService`",
    "summary": "Cloud日志已证明四个非代表窗口均为`MEMBER`；共同失败根因是Spring创建Cloud分配后的`AutoBattleTask`时报`No default constructor found`。生产构造器现显式`@Autowired`，ACK回传实际`AUTO_BATTLE`队列供UI显示，终态携带失败原因。父级审查`P0/P1/P2=0/0/0`，Cloud隔离合同`64/64`、双端main compile通过；重启双端后验收四队员显示并运行自动战斗。",
    "verification": "需复核"
  },
  {
    "id": "CR213",
  ...
  },
  {
    ...
    "number": 226,
    "owner": "Codex",
    "status": "Fresh runtime P1 repaired：Fast Exit pending/visible 竞态已修，待重启复验",
    "kind": "review",
    "domain": "修罗",
    "files": "`WindowObservationRunner`, `WindowObservationSampler`, `LocalCombatSignalMechanics`, observation DTO, `CloudWindowObservationInbox`, `CloudWholeTaskObserver`, `BattleRadarService`, `FastExpectedCombatExitProbe`, `CloudFastExpectedCombatExitCoordinator`, `AutoCombatService`",
    "summary": "2026-07-23 修罗实跑确认队长约 59 分钟产生 `7258` 次 HWND capture、`1629` 次 observation 请求，单 dialog ROI 已约 `186.54 MiB` PNG。已按批准窄边界复用 `COMBAT_SIGNAL`：`combat-flag/selection/top` 三组基线模板在本地内存匹配且模板按 run 缓存，失败为 `UNAVAILABLE`；Cloud 唯一拥有战斗状态机、连续 miss、防抖、phase/wakeup。`coordinate-strip` 仅在 exact active pathing 或普通脱战兜底时动态请求，并有 revision/sequence/time 栅栏。重复 replacement lineage、arm identity A/B 漂移均已修。最新 fresh run `remote-turn-92876022-061e-4330-95fb-70c8c294bdd3` 进一步证明 fast edge 已发布并在 `313ms` 内唤醒任务，但 no-turn observer 在 one-shot 尚未消费时用重叠 `VISIBLE` 样本复活同一战斗，导致任务按 `IN_COMBAT` 丢弃退出。Cloud 现保护 exact fast-exit pending 到消费的原子边界；消费后下一场真实 visible 仍可进入。聚焦合同 `49/49`、compile/diff-check exit `0`，须重启后验证 post-combat 直接推进且无 battleCount 复活/`discard stale`。",
    "verification": "需复核"
  },
  {
    "id": "CR227",
  ...
  },
  {
    ...
    "number": 253,
    "owner": "Codex",
    "status": "DELIVERED / 待父级审核：已恢复 HTTPS 修罗 `due -> SUMMON_SKILL_CLEANUP -> PREPARED_ACTION_READY -> task consumer -> 再 park` 接线；Cloud compile 通过，fresh 待验",
    "kind": "ready",
    "domain": "窗口/Runner",
    "files": "`XiuluoTaskV2`, `CloudWholeTaskObserver`, `TaskMaintenanceService`, `CloudWholeTaskReadyEventState`, external `dhxy-cloud-brain` turn runtime",
    "summary": "不新增平行状态：observer 仅调用既有 FIFO 的 enqueue-only 分支，再把现有 `PreparedActionJob` 放入既有 ready-event 容器；task consumer 复用原完整维护。exact run/window/round/attempt fence、cooldown、unknown-retry、snapshot、预算和清理点击逻辑保持。fresh 未通过前不得关闭。",
    "verification": "未实现/未认领"
  },
  {
    "id": "CR254",
  ...
  },
  {
    ...
    "number": 271,
    "owner": "Codex",
    "status": "STOP-RESTART-P1源码与合同测试通过：旧Observer及终态旧命令槽均不再污染fresh run，异步终态可准确退出UI运行态",
    "kind": "open",
    "domain": "通用",
  ...
  },
  {
    ...
    "number": 272,
    "owner": "Codex",
    "status": "Review：五环 Tracker/NPC 后台预计算；接任务 OPTION 点击后等待 fresh Tracker anchor；HWND 截图统一注册几何",
    "kind": "review",
    "domain": "窗口/Runner",
    "files": "external `dhxy-cloud-brain` `FiveRingTaskV2`, `NpcClickService`, `DialogService`, observer、Client `FiveRingAcceptDialogLocalOperation`, `WindowNativeBindingRefreshService`, `BoundWindowCaptureService`",
    "summary": "已接通 NPC prepared FIFO；接任务点击后不再立即截 STORY，而是 `250ms` 后每秒探测 anchor、最长 `5s`，命中才上传 fresh Tracker panel；超时先由 Cloud 重判当前 Dialog。白字结果构造 NPE 已修。窗口未缩放时拒绝将运行期错误 `812x663` 当作整图尺寸，后台 HWND 截图固定沿用注册 `1036x783`；不改 ROI、不恢复前台 Robot 兜底。双仓 compile 通过，fresh runtime 待验。",
    "verification": "需复核"
  },
  {
    "id": "CR273",
    "number": 273,
    "owner": "ZERO OWNER",
    "status": "源码与合同终审通过，fresh runtime 待验：到达路径和普通/近距离 NPC ClickSmart 均复用现有流式 FIFO；候选产生即由 Client 消费，不再等待完整 plan",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "Client observation/Runner/FIFO transport；Cloud observation ingress、`NpcClickSmartQueueStore`、`NpcClickService`、修罗/五倍接线",
    "summary": "父级增量审查 `P0/P1/P2=0/0/0`；新增 direct FIFO 合同与 Store 合计 `10/10`，双仓 compile exit `0`。未改识别器、候选顺序、模板、阈值和验证语义；首轮完整失败仍只允许一次 `cleanupAll` 和替换 FIFO。需重启双端做真实窗口验收。",
    "verification": "需复核"
  },
  {
    "id": "CR275",
    "number": 275,
    "owner": "ZERO OWNER",
    "status": "源码终审完成：撤销修罗医宝宝临时 live-test 默认值与专用启动绕行，恢复正常启动准备和维护到期判断；等待 fresh restart",
    "kind": "open",
    "domain": "修罗",
    "files": "Client `AutoBot`/metadata/launcher；Cloud `BotProperties`/host",
    "summary": "`P0/P1/P2=0/0/0`，双仓 compile exit `0`，测试开关引用清零；未回滚正式修罗维护、寻路、Runner、OCR 或 NPC ClickSmart。",
    "verification": "需复核"
  },
  {
    "id": "CR274",
    "number": 274,
    "owner": "ZERO OWNER",
    "status": "源码终审完成：Cloud 启动前同步拥有 OCR sidecar 生命周期；OCR/队伍身份不可用时 start ACK 前明确拒绝，Client 立即释放并显示真实原因；等待 fresh runtime",
    "kind": "open",
    "domain": "窗口/Runner",
    "files": "Cloud `CloudBrainApplication`/OCR lifecycle/turn start；Client `HttpsTurnClient` 错误详情",
    "summary": "`P0/P1/P2=0/0/0`，双仓 compile exit `0`。修复 09:47 OCR 未启动导致五窗口 `UNKNOWN -> SKIPPED`、空队列占用 60 秒和设置锁定；未改变 OCR/角色分配/任务业务语义。",
    "verification": "需复核"
  },
  {
    "id": "CR276",
    "number": 276,
    "owner": "ZERO OWNER",
    "status": "源码终审完成：单人任务 hover 截图成功且无 tooltip 时判 `SOLO`；五环删除冲突的手写 Spring 构造器；等待 fresh runtime",
    "kind": "open",
    "domain": "五环",
    "files": "Client `TaskType`/`LocalTeamRolePreflightService`；Cloud `FiveRingTaskV2`",
    "summary": "`P0/P1/P2=0/0/0`，双仓 compile exit `0`，五环仅剩一个完整注入构造器。规则按单人任务能力声明，不在预检中硬编码五环；截图/输入失败仍为 `UNKNOWN`。",
    "verification": "需复核"
  },
  {
    "id": "CR260",
