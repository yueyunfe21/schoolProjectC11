...
/**
 * Screenshot-only observation of the leader-side team-return signal inside one exact native-window
 * binding. It mirrors the committed {@code 0114604e} {@code findLeaderReturnSignal} /
 * {@code analyzeLeaderSignalSnapshot} mechanics (configured team area,
 * {@code images/template/status/zhao.png}, {@code returnTeamMatchRate}) but sources the frame from
 ...
 * one null. It only reports whether a waiting-member marker is visible now and where; whether the
 * leader then supplements incense, re-observes, or issues a click bundle stays with the cloud
 * business orchestration.
 */
@Slf4j
@Service
public final class TeamReturnLeaderSignalLocalObservationMechanics {
    ...
    public TeamReturnLeaderSignalLocalObservationMechanics(BoundWindowCaptureService captureService,
    ...
    /**
     * Observes the configured leader team-return signal region for one exact native-window binding.
     *
    ...
    public ObservationResult observe(WindowNativeBinding binding) {
        ...
        try {
            ...
            log.warn("[team-return] leader signal local observation capture failed: hwnd={} title={} reason={}",
        ...
        try {
            try {
                template = ImageIO.read(Path.of(LEADER_RETURN_SIGNAL_PATH).toFile());
            } catch (IOException e) {
                log.warn("[team-return] leader signal local observation template unreadable: path={} reason={}",
                        LEADER_RETURN_SIGNAL_PATH, e.getMessage(), e);
            ...
            log.warn("[team-return] leader signal local observation mechanics failed: hwnd={} title={} reason={}",
                    binding.getNativeHandle(), binding.getTitle(), e.getMessage(), e);
            return new ObservationResult(State.MECHANICS_FAILED, null, null, null);
        } finally {
            frame.flush();
